...
namespace Test4.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult ExternalLogin(string provider, string returnUrl)
        {
            ...
            //        {
            //            RedirectUri = Url.Action("LoginCallbackAdfs", "Account", new { provider })
            //        },
            //        provider);
            if (provider == "Avensia ADFS")
            {
                ctx.Authentication.Challenge(
                    new AuthenticationProperties
                    {
                        RedirectUri = Url.Action("LoginCallbackAvensiaAdfs", "Account", new { provider })
                    },
                    provider);
            }
            else if (provider == "Samhall ADFS")
            {
                ctx.Authentication.Challenge(new AuthenticationProperties
                {
                    RedirectUri = Url.Action("LoginCallbackSamhallAdfs", "Account", new { provider })
                },
                provider);
            }
        ...
        public ActionResult LoginCallbackAdfs(string provider)
        {
            var ctx = Request.GetOwinContext();
        ...
        public ActionResult LoginCallbackAvensiaAdfs(string provider)
        {
            var ctx = Request.GetOwinContext();
            var result = ctx.Authentication.AuthenticateAsync("ExternalCookie").Result;
            ctx.Authentication.SignOut("ExternalCookie");
        ...
        public ActionResult LoginCallbackSamhallAdfs(string provider)
        {
            var ctx = Request.GetOwinContext();
            var result = ctx.Authentication.AuthenticateAsync("ExternalCookie").Result;
            ctx.Authentication.SignOut("ExternalCookie");
